  ...
  async store(req, res) {
    ...
    try {
      ...
      const logData = {
        book_id,
        student_matricula,
        loan_date: loan_date.toISO(),
        return_date: return_date.toISO(),
        returned: false,
        pendent: false,
        book_name,
        student_name,
  ...
  async update(req, res) {
    ...
    try {
      ...
      if (loanToUpdate) {
        ...
        loanToUpdate.return_date = return_date
          ? return_date
          : loanToUpdate.return_date;
        loanToUpdate.book_id = book_id ? book_id : loanToUpdate.book_id;
        loanToUpdate.student_matricula = student_matricula
          ? student_matricula
          : loanToUpdate.student_matricula;
  ...
  async devolution(req, res) {
    ...
    try {
      ...
      if (loanToReturn.returned == false) loanToReturn.returned = true;
        ...
        return res
          .status(400)
          .json({ message: "Esse empréstimo já foi retornado." });
