...
class StudentController {
  ...
  async show(req, res) {
    ...
    try {
      ...
      if (student) {
        ...
        return res
          .status(404)
          .json({ message: "Aluno não existe no banco de dados. " });
      }
    } catch (error) {
      console.error(error);
      return res.status(500).json({
  ...
  async update(req, res) {
    ...
    try {
      ...
      if (studentToUpdate) {
        studentToUpdate.matricula = matricula
          ? matricula
          : studentToUpdate.matricula;
        studentToUpdate.full_name = full_name
          ? full_name
        ...
        studentToUpdate.can_borrow = can_borrow
          ? can_borrow
          : studentToUpdate.can_borrow;
